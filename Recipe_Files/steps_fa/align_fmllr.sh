...
if [ $stage -le 3 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/align_pass2.JOB.log \
    gmm-align-compiled $scale_opts --beam=$beam --retry-beam=$retry_beam --write-per-frame-acoustic-loglikes="ark,t:$dir/log/per_frame_logprobs.JOB.ark" --careful=$careful "$mdl_cmd" \
